...
ConstantBuffer<SsrMedianBlurCb> SsrBlur	: register(b0);
...
{
    const float2 uv = (cin.DispatchThreadId.xy + 0.5f) * SsrBlur.InvDimensions;
    ...
    for (int dX = -2; dX <= 2; ++dX)
    {
        ...
        for (int dY = -2; dY <= 2; ++dY)
        {
            ...
            v[(dX + 2) * 5 + (dY + 2)] = Input.SampleLevel(LinearSampler, uv + offset * SsrBlur.InvDimensions, 0);
        }
    }
    ...
    Output[cin.DispatchThreadId.xy] = v[12];
}

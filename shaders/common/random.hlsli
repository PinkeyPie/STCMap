...
float3 GetCosHemisphereSample(inout uint randSeed, float3 hitNorm)
{
    ...
    // Cosine weighted hemisphere sample from ENG
    float3 bitangent = GetPerpendicularVector(hitNorm);
    float3 tangent = cross(bitangent, hitNorm);
    float r = sqrt(randVal.x);
    float phi = 2.f * 3.14159265f * randVal.y;
...
{
    float2 h = float2(NextRand(randSeed), NextRand(randSeed)) * float2(1.f, 2.f * PI) - float2(1.f, 0.f);
    float phi = h.y;

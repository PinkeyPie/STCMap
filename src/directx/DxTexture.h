...
public:
	...
	void Resize(DxContext* context, uint32 newWidth, uint32 newHeight, D3D12_RESOURCE_STATES initialState = D3D12_RESOURCE_STATE_COMMON);

	static DxTexture Create(DxContext* context, D3D12_RESOURCE_DESC textureDesc, D3D12_SUBRESOURCE_DATA* subresourceData, uint32 numSubresources, D3D12_RESOURCE_STATES initialState = D3D12_RESOURCE_STATE_COMMON);
	static DxTexture Create(DxContext* context, const void* data, uint32 width, uint32 height, DXGI_FORMAT format, bool allowUnorderedAccess = false, D3D12_RESOURCE_STATES initialState = D3D12_RESOURCE_STATE_COMMON);
	static DxTexture CreateDepth(DxContext* context, uint32 width, uint32 height, DXGI_FORMAT format);
...
private:
	void UploadSubresourceData(DxContext* context, D3D12_SUBRESOURCE_DATA* subresourceData, uint32 firstSubresource, uint32 numSubresources);
